...
export default function QuickView({ selectedProduct, selectedLoading }) {
    ...
    return (
        <form className="reviews mt-6 w-[50rem] border-2 border-gray-700 border-solid rounded-lg p-5 mx-auto bg-white" onSubmit={handleSubmit(updateAProduct)}>
            <img src={selectedProduct.productImage} alt="" className="object-contain w-full h-80" />
            <input {...register("title", { required: true })} type="text" defaultValue={selectedProduct.title} placeholder="enter product name" className="border-gray-800 border-solid bg-gray-100 border-2 rounded-lg w-full p-3 text-xl text-gray-800" />
