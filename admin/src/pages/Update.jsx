...
export default function Update() {
    ...
    const { products, product_loading, selected_product_loading, product_success, product_error, product_message } = useSelector(state => state.product)
    const selectedProduct = products.find(product => product.slug === slug)
    const dispatch = useDispatch()

    useEffect(() => {
        if (product_success) toast(product_message, { type: 'success', autoClose: 2000 })
